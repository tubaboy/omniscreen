...
> [!TIP]
> **專業推薦：使用主機名稱 (.local) 代替 IP**
> 如果你的電腦名稱是 `linux-mint` 且有安裝 Avahi (Linux) 或 Bonjour (Windows/Mac)，你可以直接使用 `linux-mint.local` 代替 IP。
> 這樣設定的好處是：即使路由器重啟導致 IP 變動，你的連結依然有效。
> ```env
> NEXT_PUBLIC_API_URL=http://linux-mint.local:3001/api
> MINIO_ENDPOINT=http://linux-mint.local:9000
> ```
